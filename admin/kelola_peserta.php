...
if (isset($_POST['tambah_peserta'])) {
    ...
    // Query untuk menyimpan peserta baru ke database
    // Menggunakan prepared statement untuk keamanan
    $query = "INSERT INTO users (username, email, password, role) VALUES (?, ?, ?, 'peserta')";
    $stmt = $koneksi->prepare($query);
    $stmt->bind_param("sss", $nama_peserta, $email, $password);

    if ($stmt->execute()) {
        $message = "Peserta berhasil ditambahkan!";
        $message_class = "success";
    } else {
        $message = "Terjadi kesalahan saat menambahkan peserta: " . $stmt->error;
        $message_class = "error";
    }
    $stmt->close();
}
...
if (isset($_GET['hapus_peserta'])) {
    ...
    // Query untuk menghapus peserta dari database
    // Menggunakan prepared statement untuk keamanan
    $query = "DELETE FROM users WHERE id = ? AND role = 'peserta'";
    $stmt = $koneksi->prepare($query);
    ...
    if ($stmt->execute()) {
        ...
        $message = "Terjadi kesalahan saat menghapus peserta: " . $stmt->error;
        $message_class = "error";
    }
    $stmt->close();
}
...
<head>
    ...
    <title>Kelola Peserta - Admin Portal</title>
    ...
    <style>
        ...
        .form-tambah-peserta input[type="text"],
        .form-tambah-peserta input[type="email"],
        .form-tambah-peserta input[type="password"] {
        ...
        .form-tambah-peserta button[type="submit"] {
            width: 100%;
            padding: 12px;
            background-color: #287bff;
            color: white;
        ...
        .form-tambah-peserta button[type="submit"]:hover {
            background-color: #004bb5;
            transform: translateY(-2px);
        }

        /* Tombol untuk Tambah Peserta */
        .btn-tambah-peserta {
...
<body>
    ...
    <div class="navbar">
        ...
        <div class="navbar-right">
            <ul class="menu">
                ...
                <li><a href="kelola_peserta.php" class="active">Kelola Peserta</a></li> <li><a href="kelola_juri.php">Kelola Juri</a></li>
    ...
    <div class="content">
        ...
        <div id="formTambahPeserta" class="form-tambah-peserta" style="display:none;">
            <h3>Tambah Peserta Baru</h3>
            <form method="POST">
                <div class="form-group">
                    <label for="nama_peserta">Nama Peserta:</label>
                    <input type="text" id="nama_peserta" name="nama_peserta" placeholder="Nama Peserta" required>
                </div>
                <div class="form-group">
                    <label for="email">Email Peserta:</label>
                    <input type="email" id="email" name="email" placeholder="Email Peserta" required>
                </div>
                <div class="form-group">
        ...
        <a href="#" class="btn-tambah-peserta" onclick="document.getElementById('formTambahPeserta').style.display='block'; return false;">Tambah Peserta</a>
        ...
        <div class="table-responsive">
            <table class="data-table">
                ...
                <tbody>
                    <?php
                    // Query untuk menampilkan semua peserta
                    // Pastikan koneksi $koneksi masih aktif di sini.
                    $query_peserta_list = $koneksi->query("SELECT id, username, email, created_at FROM users WHERE role = 'peserta' ORDER BY username ASC");
                    if ($query_peserta_list->num_rows > 0) {
                        while ($data = $query_peserta_list->fetch_assoc()) {
                            echo '
                            <tr>
                                ...
                                <td>
                                    <a href="?hapus_peserta=true&id_peserta=' . htmlspecialchars($data['id']) . '" class="btn-action btn-delete" onclick="return confirm(\'Apakah Anda yakin ingin menghapus peserta ini?\')">Hapus</a>
                                </td>
                            </tr>';
                        }
                    } else {
                        echo '<tr><td colspan="5" style="text-align: center;">Belum ada peserta terdaftar.</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
